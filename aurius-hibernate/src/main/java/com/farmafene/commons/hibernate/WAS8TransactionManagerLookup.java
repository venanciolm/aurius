...
/**
 * Support for proprietary interfaces for registering synchronizations in
 * WebSphere 6.
 * 
 * @author Gavin King
 */
public class WAS8TransactionManagerLookup implements TransactionManagerLookup {
	...
	public static class TransactionManagerAdapter implements TransactionManager {
		...
		private TransactionManagerAdapter(Properties props) {
			...
			try {
				synchronizationCallbackClass = Class
						.forName("com.ibm.websphere.jtaextensions.SynchronizationCallback");
				Class extendedJTATransactionClass = Class
						.forName("com.ibm.websphere.jtaextensions.ExtendedJTATransaction");
				registerSynchronizationMethod = extendedJTATransactionClass
						.getMethod(
		...
		public class TransactionAdapter implements Transaction {
			...
			private TransactionAdapter(Properties props) {
				try {
					extendedJTATransaction = NamingHelper.getInitialContext(
							props).lookup(
			...
			public void registerSynchronization(
				...
				final Object synchronizationCallback = Proxy.newProxyInstance(
						...
						new Class[] { synchronizationCallbackClass }, ih);
			...
			private Object getLocalId() {
				try {
					return getLocalIdMethod.invoke(extendedJTATransaction,
							(Object[]) null);
				} catch (Exception e) {
					throw new HibernateException(e);
				}
